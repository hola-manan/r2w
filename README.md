...
```
backend/   FastAPI + SQLAlchemy (SQLite) + Gemini (Vertex AI Express)
...
- A Vertex AI Express mode API key (only needed to run the agents/engines *live*; the test suite
...
cp .env.example .env        # set GOOGLE_API_KEY (+ optional GOOGLE_MODEL)
